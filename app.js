...
function renderReferencePreview(){
  ...
  box.innerHTML=`<div><span>MODÈLE QUE TU AVAIS EN TÊTE</span><strong>${escapeHtml(ref.brand)} ${escapeHtml(ref.name)}</strong><small>${ref.generation} · ${labelCushion(ref.cushion)} · ${labelFoam(ref.foam)} · drop ${ref.drop} mm · ${ref.carbon?'carbone':'sans carbone'}</small></div><div class="reference-price"><span>Prix actuellement connu</span><strong>${Number(ref.price).toLocaleString('fr-FR')} €</strong><small>${ref.isReal?'snapshot web sourcé':'donnée locale'}</small></div><div class="reference-arrow">→</div><div class="reference-goal"><span>OBJECTIF</span><strong>Trouver le même esprit moins cher</strong><small>Les résultats doivent coûter moins de ${Number(ref.price).toLocaleString('fr-FR')} € et conserver au moins 50 % de similarité technique.</small></div>`;
}
...
function render(){
  ...
  $('#resultsGrid').innerHTML=rows.map((shoe,i)=>{
    ...
    const current=currentFamilyShoe(shoe);
    const saving=current && current.id!==shoe.id ? Math.round((current.price-shoe.price)*100)/100 : 0;
    const fitGap=current && current.id!==shoe.id ? fitGapVsCurrent(shoe,f) : 0;
    ...
    return `<article class="shoe-card ${isOld?'old-generation-card':'current-generation-card'} ${reference?'reference-result-card':''}">
      ...
      <div class="shoe-top"><div><div class="shoe-brand">${shoe.brand}</div><div class="shoe-name">${shoe.name}</div><div class="source-line">${sourceBadge(shoe)}${shoe.sourceLabel?`<span class="snapshot-badge">${shoe.sourceLabel}</span>`:''}${shoe.checkedAt?`<span class="snapshot-badge">collecté ${new Date(shoe.checkedAt).toLocaleDateString('fr-FR')}</span>`:''}</div><div class="shoe-gen">${isOld && shoe.compare ? `Face à ${shoe.compare.current}` : 'Version actuelle'}${i===0?' · recommandée':''}</div></div><div class="score-bubble"><strong>${score}</strong><span>/100</span></div></div>
      <div class="shoe-visual"><div class="mini-shoe">${shoe.brand}</div><span class="deal-pill">${referenceDiscountText(shoe)}</span></div>
      ...
      ${isOld && current ? `<div class="old-gen-highlight"><strong>${saving>=0?`${Number(saving).toLocaleString('fr-FR')} € moins chère que N`:`N est ${Number(-saving).toLocaleString('fr-FR')} € moins chère aujourd’hui`}</strong><span>${fitGap===0?'Fit calculé identique':`${Math.abs(fitGap)} pt${Math.abs(fitGap)>1?'s':''} de Fit ${fitGap>0?'en faveur de N':'en faveur de l’ancienne'}`}</span></div>` : '<div class="current-gen-note">Référence technique actuelle de la famille</div>'}
      ...
      <div class="tags"><span class="tag">Drop ${shoe.drop} mm</span><span class="tag">${labelCushion(shoe.cushion)}</span><span class="tag">${shoe.plateType|| (shoe.carbon?'Carbone':'Sans plaque')}</span></div>
      <div class="shoe-pricing"><div class="price-block"><span class="old-price">${shoe.launch?`${refPriceLabel(shoe)} ${shoe.launch} €`:`${shoe.sourceLabel||"Source web"}`}</span><span class="current-price">${shoe.price} €</span><span class="shop">chez ${shoe.shop} · ${shoe.sizeStockKnown===false?'taille à vérifier':`pointure ${f.size}`}</span></div><span class="badge ${shoe.deal>=90?'badge-green':''}">${dealBadgeLabel(shoe.deal)}</span></div>
