...
const state = {rank:'balanced', mode:'similar', searchLaunched:false, searchDirty:false, showAllResults:false, generations:new Set(['N','N−1','N−2','N−3']), history:[], referenceShoeId:null, strava:{connected:false,configured:null,athlete:null,shoes:[]}};
...
function getFilters(){
  ...
  return {
    terrain:experienceMode?$('#experienceTerrain').value:$('#terrain').value,
...
function render(){
  ...
  let rows=shoes.map(shoe=>({...shoe,fit:scoreFit(shoe,f),similarity:reference?similarityScore(reference,shoe):null,savingVsReference:reference?referenceSavings(reference,shoe):null}))
    ...
    .filter(shoe=>state.mode!=='experience' || criteriaSurfaceCompatible(f.terrain,shoe.terrain))
