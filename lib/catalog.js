...
function mapModel(model,offers=[]){
  ...
  return {
    ...
    avg90:Number(model.avg90_price||price),best:Number(model.best_price||price),price,shop:best?.retailer_name||'Marchand',sizes:sizes.length?sizes:['*'],sizeStockKnown:sizes.length>0,
    weightRange:model.weight_range||null,weightGrams:model.weight_grams?Number(model.weight_grams):null,plateType:model.plate_type||(model.carbon_plate?'Carbone':'Aucune'),deal:dealScore(price,ref,Number(model.best_price||price)),
    reviews:model.review_points||[],offers:sorted.map(o=>[o.retailer_name||'Marchand',Number(o.price)]),isReal:true,
    checkedAt:best?.checked_at||model.updated_at||new Date().toISOString(),sourceUrl:best?.product_url||model.source_url||null,sourceLabel:best?.retailer_name||'Supabase'
  };
}
